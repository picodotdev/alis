...
function prepare_partition() {
    set +e
    mountpoint -q ${ALISMNT}/boot
    if [ $? == 0 ]; then
        umount ${ALISMNT}/boot
    fi
    mountpoint -q ${ALISMNT}
    if [ $? == 0 ]; then
        umount ${ALISMNT}
    fi
...
function partition() {
    ...
    if [ "$FILE_SYSTEM_TYPE" == "btrfs" ]; then
        ...
        for I in "${BTRFS_SUBVOLUMES_MOUNTPOINTS[@]}"; do
            ...
            btrfs subvolume create "${ALISMNT}/${SUBVOLUME[1]}"
        done
        umount ${ALISMNT}
    fi
    ...
    if [ -n "$SWAP_SIZE" ]; then
        if [ "$FILE_SYSTEM_TYPE" == "btrfs" ]; then
            ...
            truncate -s 0 ${ALISMNT}$SWAPFILE
            chattr +C ${ALISMNT}$SWAPFILE
            btrfs property set ${ALISMNT}$SWAPFILE compression none
        fi

        dd if=/dev/zero of=${ALISMNT}$SWAPFILE bs=1M count=$SWAP_SIZE status=progress
        chmod 600 ${ALISMNT}$SWAPFILE
        mkswap ${ALISMNT}$SWAPFILE
    fi
...
function install() {
    ...
    pacstrap ${ALISMNT} base base-devel linux linux-firmware "${PACKAGES[@]}"

    sed -i 's/#Color/Color/' ${ALISMNT}/etc/pacman.conf
    if [ "$PACMAN_PARALLEL_DOWNLOADS" == "true" ]; then
        sed -i 's/#ParallelDownloads/ParallelDownloads/' ${ALISMNT}/etc/pacman.conf
    else
        sed -i 's/#ParallelDownloads\(.*\)/#ParallelDownloads\1\nDisableDownloadTimeout/' ${ALISMNT}/etc/pacman.conf
    fi

    if [ "$REFLECTOR" == "true" ]; then
        pacman_install "reflector"
        cat <<EOT > ${ALISMNT}/etc/xdg/reflector/reflector.conf
${COUNTRIES[@]}
--latest 25
--age 24
--protocol https
...
EOT
        arch-chroot ${ALISMNT} reflector "${COUNTRIES[@]}" --latest 25 --age 24 --protocol https --completion-percent 100 --sort rate --save /etc/pacman.d/mirrorlist
        arch-chroot ${ALISMNT} systemctl enable reflector.timer
    fi

    if [ "$PACKAGES_MULTILIB" == "true" ]; then
        sed -z -i 's/#\[multilib\]\n#/[multilib]\n/' ${ALISMNT}/etc/pacman.conf
    fi
}
...
function configuration() {
    ...
    genfstab -U ${ALISMNT} >> ${ALISMNT}/etc/fstab

    if [ -n "$SWAP_SIZE" ]; then
        echo "# swap" >> ${ALISMNT}/etc/fstab
        echo "$SWAPFILE none swap defaults 0 0" >> ${ALISMNT}/etc/fstab
        echo "" >> ${ALISMNT}/etc/fstab
    fi

    if [ "$DEVICE_TRIM" == "true" ]; then
        if [ "$FILE_SYSTEM_TYPE" == "f2fs" ]; then
            sed -i 's/relatime/noatime,nodiscard/' ${ALISMNT}/etc/fstab
        else
            sed -i 's/relatime/noatime/' ${ALISMNT}/etc/fstab
        fi
        arch-chroot ${ALISMNT} systemctl enable fstrim.timer
    fi

    arch-chroot ${ALISMNT} ln -s -f $TIMEZONE /etc/localtime
    arch-chroot ${ALISMNT} hwclock --systohc
    for LOCALE in "${LOCALES[@]}"; do
        sed -i "s/#$LOCALE/$LOCALE/" /etc/locale.gen
        sed -i "s/#$LOCALE/$LOCALE/" ${ALISMNT}/etc/locale.gen
    done
    for VARIABLE in "${LOCALE_CONF[@]}"; do
        #localectl set-locale "$VARIABLE"
        echo -e "$VARIABLE" >> ${ALISMNT}/etc/locale.conf
    done
    locale-gen
    arch-chroot ${ALISMNT} locale-gen
    echo -e "$KEYMAP\n$FONT\n$FONT_MAP" > ${ALISMNT}/etc/vconsole.conf
    echo $HOSTNAME > ${ALISMNT}/etc/hostname
    ...
    arch-chroot ${ALISMNT} mkdir -p "/etc/X11/xorg.conf.d/"
    cat <<EOT > ${ALISMNT}/etc/X11/xorg.conf.d/00-keyboard.conf
...
EOT
    ...
    if [ -n "$SWAP_SIZE" ]; then
        echo "vm.swappiness=10" > ${ALISMNT}/etc/sysctl.d/99-sysctl.conf
    fi

    printf "$ROOT_PASSWORD\n$ROOT_PASSWORD" | arch-chroot ${ALISMNT} passwd
}
...
function mkinitcpio_configuration() {
    ...
    if [ "$DISPLAY_DRIVER" == "intel" ]; then
        ...
        if [ -n "$OPTIONS" ]; then
            echo "options i915 $OPTIONS" > ${ALISMNT}/etc/modprobe.d/i915.conf
        fi
    fi
    ...
    arch-chroot ${ALISMNT} sed -i "s/^HOOKS=(.*)$/HOOKS=($HOOKS)/" /etc/mkinitcpio.conf
    arch-chroot ${ALISMNT} sed -i "s/^MODULES=(.*)/MODULES=($MODULES)/" /etc/mkinitcpio.conf

    if [ "$KERNELS_COMPRESSION" != "" ]; then
        arch-chroot ${ALISMNT} sed -i 's/^#COMPRESSION="'"$KERNELS_COMPRESSION"'"/COMPRESSION="'"$KERNELS_COMPRESSION"'"/' /etc/mkinitcpio.conf
    fi
...
function users() {
    ...
    arch-chroot ${ALISMNT} sed -i 's/# %wheel ALL=(ALL:ALL) ALL/%wheel ALL=(ALL:ALL) ALL/' /etc/sudoers

    pacman_install "xdg-user-dirs"

    if [ "$SYSTEMD_HOMED" == "true" ]; then
        arch-chroot ${ALISMNT} systemctl enable systemd-homed.service

        cat <<EOT > "${ALISMNT}/etc/pam.d/nss-auth"
#%PAM-1.0
...
EOT
        ...
        cat <<EOT > "${ALISMNT}/etc/pam.d/system-auth"
#%PAM-1.0
...
function create_user_homectl() {
    ...
    local IMAGE_PATH="--image-path=${ALISMNT}/home/$USER"
    local FS_TYPE=""
    local CIFS_DOMAIN=""
    local CIFS_USERNAME=""
    local CIFS_SERVICE=""
    ...
    if [ "$SYSTEMD_HOMED_STORAGE" == "luks" ]; then
        local IMAGE_PATH="--image-path="${ALISMNT}/home/$USER.home""
    fi
    if [ "$SYSTEMD_HOMED_STORAGE" == "cifs" ]; then
    ...
    sleep 10 # #151 avoid Operation on home <USER> failed: Transport endpoint is not conected.
    cp -a "/var/lib/systemd/home/." "${ALISMNT}/var/lib/systemd/home/"
}
...
function create_user_useradd() {
    ...
    local PASSWORD=$2
    local USER_GROUPS=$3
    arch-chroot ${ALISMNT} useradd -m -G "$USER_GROUPS" -s /bin/bash $USER
    printf "$USER_PASSWORD\n$USER_PASSWORD" | arch-chroot ${ALISMNT} passwd $USER
}
...
function user_add_groups() {
    ...
    if [ "$SYSTEMD_HOMED" == "true" ]; then
        ...
        arch-chroot ${ALISMNT} usermod -a -G "$USER_GROUPS" "$USER"
    fi
}

function user_add_groups_lightdm() {
    arch-chroot ${ALISMNT} groupadd -r "autologin"
...
function mkinitcpio() {
    ...
    arch-chroot ${ALISMNT} mkinitcpio -P
}
...
function network() {
    ...
    arch-chroot ${ALISMNT} systemctl enable NetworkManager.service
}
...
function virtualbox() {
    ...
    arch-chroot ${ALISMNT} systemctl enable vboxservice.service
...
function vmware() {
    ...
    arch-chroot ${ALISMNT} systemctl enable vmtoolsd.service
}
...
function bootloader() {
    ...
    arch-chroot ${ALISMNT} systemctl set-default multi-user.target
}

function bootloader_grub() {
    pacman_install "grub dosfstools"
    arch-chroot ${ALISMNT} sed -i 's/GRUB_DEFAULT=0/GRUB_DEFAULT=saved/' /etc/default/grub
    arch-chroot ${ALISMNT} sed -i 's/#GRUB_SAVEDEFAULT="true"/GRUB_SAVEDEFAULT="true"/' /etc/default/grub
    arch-chroot ${ALISMNT} sed -i -E 's/GRUB_CMDLINE_LINUX_DEFAULT="(.*) quiet"/GRUB_CMDLINE_LINUX_DEFAULT="\1"/' /etc/default/grub
    arch-chroot ${ALISMNT} sed -i 's/GRUB_CMDLINE_LINUX=""/GRUB_CMDLINE_LINUX="'"$CMDLINE_LINUX"'"/' /etc/default/grub
    echo "" >> ${ALISMNT}/etc/default/grub
    echo "# alis" >> ${ALISMNT}/etc/default/grub
    echo "GRUB_DISABLE_SUBMENU=y" >> ${ALISMNT}/etc/default/grub

    if [ "$BIOS_TYPE" == "uefi" ]; then
        pacman_install "efibootmgr"
        arch-chroot ${ALISMNT} grub-install --target=x86_64-efi --bootloader-id=grub --efi-directory=$ESP_DIRECTORY --recheck
        #arch-chroot ${ALISMNT} efibootmgr --create --disk $DEVICE --part $PARTITION_BOOT_NUMBER --loader /EFI/grub/grubx64.efi --label "GRUB Boot Manager"
    fi
    if [ "$BIOS_TYPE" == "bios" ]; then
        arch-chroot ${ALISMNT} grub-install --target=i386-pc --recheck $DEVICE
    fi

    arch-chroot ${ALISMNT} grub-mkconfig -o "$BOOT_DIRECTORY/grub/grub.cfg"

    if [ "$VIRTUALBOX" == "true" ]; then
        echo -n "\EFI\grub\grubx64.efi" > "${ALISMNT}$ESP_DIRECTORY/startup.nsh"
    fi
}

function bootloader_refind() {
    pacman_install "refind"
    arch-chroot ${ALISMNT} refind-install

    arch-chroot ${ALISMNT} rm /boot/refind_linux.conf
    arch-chroot ${ALISMNT} sed -i 's/^timeout.*/timeout 5/' "$ESP_DIRECTORY/EFI/refind/refind.conf"
    arch-chroot ${ALISMNT} sed -i 's/^#scan_all_linux_kernels.*/scan_all_linux_kernels false/' "$ESP_DIRECTORY/EFI/refind/refind.conf"
    #arch-chroot ${ALISMNT} sed -i 's/^#default_selection "+,bzImage,vmlinuz"/default_selection "+,bzImage,vmlinuz"/' "$ESP_DIRECTORY/EFI/refind/refind.conf"
    ...
    cat <<EOT >> "${ALISMNT}$ESP_DIRECTORY/EFI/refind/refind.conf"
# alis
...
EOT
    ...
    if [ "$VIRTUALBOX" == "true" ]; then
        echo -n "\EFI\refind\refind_x64.efi" > "${ALISMNT}$ESP_DIRECTORY/startup.nsh"
    fi
}

function bootloader_systemd() {
    arch-chroot ${ALISMNT} systemd-machine-id-setup
    arch-chroot ${ALISMNT} bootctl install

    arch-chroot ${ALISMNT} mkdir -p "$ESP_DIRECTORY/loader/"
    arch-chroot ${ALISMNT} mkdir -p "$ESP_DIRECTORY/loader/entries/"

    cat <<EOT > "${ALISMNT}$ESP_DIRECTORY/loader/loader.conf"
# alis
timeout 5
...
EOT
    ...
    #arch-chroot ${ALISMNT} systemctl enable systemd-boot-update.service

    arch-chroot ${ALISMNT} mkdir -p "/etc/pacman.d/hooks/"

    cat <<EOT > "${ALISMNT}/etc/pacman.d/hooks/systemd-boot.hook"
[Trigger]
Type = Package
Operation = Upgrade
Target = systemd
...
EOT
    ...
    echo "title Arch Linux" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux.conf"
    echo "efi /vmlinuz-linux" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux.conf"
    if [ -n "$SYSTEMD_MICROCODE" ]; then
        echo "initrd $SYSTEMD_MICROCODE" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux.conf"
    fi
    echo "initrd /initramfs-linux.img" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux.conf"
    echo "options initrd=initramfs-linux.img $CMDLINE_LINUX_ROOT rw $CMDLINE_LINUX" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux.conf"

    echo "title Arch Linux (terminal)" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-terminal.conf"
    echo "efi /vmlinuz-linux" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-terminal.conf"
    if [ -n "$SYSTEMD_MICROCODE" ]; then
        echo "initrd $SYSTEMD_MICROCODE" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-terminal.conf"
    fi
    echo "initrd /initramfs-linux.img" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-terminal.conf"
    echo "options initrd=initramfs-linux.img $CMDLINE_LINUX_ROOT rw $CMDLINE_LINUX systemd.unit=multi-user.target" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-terminal.conf"

    echo "title Arch Linux (fallback)" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-fallback.conf"
    echo "efi /vmlinuz-linux" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-fallback.conf"
    if [ -n "$SYSTEMD_MICROCODE" ]; then
        echo "initrd $SYSTEMD_MICROCODE" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-fallback.conf"
    fi
    echo "initrd /initramfs-linux-fallback.img" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-fallback.conf"
    echo "options initrd=initramfs-linux-fallback.img $CMDLINE_LINUX_ROOT rw $CMDLINE_LINUX" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-fallback.conf"

    if [[ $KERNELS =~ .*linux-lts.* ]]; then
        echo "title Arch Linux (lts)" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-lts.conf"
        echo "efi /vmlinuz-linux-lts" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-lts.conf"
        if [ -n "$SYSTEMD_MICROCODE" ]; then
            echo "initrd $SYSTEMD_MICROCODE" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-lts.conf"
        fi
        echo "initrd /initramfs-linux-lts.img" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-lts.conf"
        echo "options initrd=initramfs-linux-lts.img $CMDLINE_LINUX_ROOT rw $CMDLINE_LINUX" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-lts.conf"

        echo "title Arch Linux (lts, terminal)" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-lts-terminal.conf"
        echo "efi /vmlinuz-linux-lts" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-lts-terminal.conf"
        if [ -n "$SYSTEMD_MICROCODE" ]; then
            echo "initrd $SYSTEMD_MICROCODE" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-lts-terminal.conf"
        fi
        echo "initrd /initramfs-linux-lts.img" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-lts-terminal.conf"
        echo "options initrd=initramfs-linux-lts.img $CMDLINE_LINUX_ROOT rw $CMDLINE_LINUX systemd.unit=multi-user.target" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-lts-terminal.conf"

        echo "title Arch Linux (lts-fallback)" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-lts-fallback.conf"
        echo "efi /vmlinuz-linux-lts" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-lts-fallback.conf"
        if [ -n "$SYSTEMD_MICROCODE" ]; then
            echo "initrd $SYSTEMD_MICROCODE" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-lts-fallback.conf"
        fi
        echo "initrd /initramfs-linux-lts-fallback.img" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-lts-fallback.conf"
        echo "options initrd=initramfs-linux-lts-fallback.img $CMDLINE_LINUX_ROOT rw $CMDLINE_LINUX" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-lts-fallback.conf"
    fi

    if [[ $KERNELS =~ .*linux-hardened.* ]]; then
        echo "title Arch Linux (hardened)" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-hardened.conf"
        echo "efi /vmlinuz-linux-hardened" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-hardened.conf"
        if [ -n "$SYSTEMD_MICROCODE" ]; then
            echo "initrd $SYSTEMD_MICROCODE" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-hardened.conf"
        fi
        echo "initrd /initramfs-linux-hardened.img" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-hardened.conf"
        echo "options initrd=initramfs-linux-hardened.img $CMDLINE_LINUX_ROOT rw $CMDLINE_LINUX" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-hardened.conf"

        echo "title Arch Linux (hardened, terminal)" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-hardened-terminal.conf"
        echo "efi /vmlinuz-linux-hardened" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-hardened-terminal.conf"
        if [ -n "$SYSTEMD_MICROCODE" ]; then
            echo "initrd $SYSTEMD_MICROCODE" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-hardened-terminal.conf"
        fi
        echo "initrd /initramfs-linux-hardened.img" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-hardened-terminal.conf"
        echo "options initrd=initramfs-linux-hardened.img $CMDLINE_LINUX_ROOT rw $CMDLINE_LINUX systemd.unit=multi-user.target" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-hardened-terminal.conf"

        echo "title Arch Linux (hardened-fallback)" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-hardened-fallback.conf"
        echo "efi /vmlinuz-linux-hardened" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-hardened-fallback.conf"
        if [ -n "$SYSTEMD_MICROCODE" ]; then
            echo "initrd $SYSTEMD_MICROCODE" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-hardened-fallback.conf"
        fi
        echo "initrd /initramfs-linux-hardened-fallback.img" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-hardened-fallback.conf"
        echo "options initrd=initramfs-linux-hardened-fallback.img $CMDLINE_LINUX_ROOT rw $CMDLINE_LINUX" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-hardened-fallback.conf"
    fi

    if [[ $KERNELS =~ .*linux-zen.* ]]; then
        echo "title Arch Linux (zen)" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-zen.conf"
        echo "efi /vmlinuz-linux-zen" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-zen.conf"
        if [ -n "$SYSTEMD_MICROCODE" ]; then
            echo "initrd $SYSTEMD_MICROCODE" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-zen.conf"
        fi
        echo "initrd /initramfs-linux-zen.img" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-zen.conf"
        echo "options initrd=initramfs-linux-zen.img $CMDLINE_LINUX_ROOT rw $CMDLINE_LINUX" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-zen.conf"

        echo "title Arch Linux (zen, terminal)" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-zen-terminal.conf"
        echo "efi /vmlinuz-linux-zen" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-zen-terminal.conf"
        if [ -n "$SYSTEMD_MICROCODE" ]; then
            echo "initrd $SYSTEMD_MICROCODE" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-zen-terminal.conf"
        fi
        echo "initrd /initramfs-linux-zen.img" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-zen-terminal.conf"
        echo "options initrd=initramfs-linux-zen.img $CMDLINE_LINUX_ROOT rw $CMDLINE_LINUX systemd.unit=multi-user.target" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-zen-terminal.conf"

        echo "title Arch Linux (zen-fallback)" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-zen-fallback.conf"
        echo "efi /vmlinuz-linux-zen" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-zen-fallback.conf"
        if [ -n "$SYSTEMD_MICROCODE" ]; then
            echo "initrd $SYSTEMD_MICROCODE" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-zen-fallback.conf"
        fi
        echo "initrd /initramfs-linux-zen-fallback.img" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-zen-fallback.conf"
        echo "options initrd=initramfs-linux-zen-fallback.img $CMDLINE_LINUX_ROOT rw $CMDLINE_LINUX" >> "${ALISMNT}$ESP_DIRECTORY/loader/entries/archlinux-zen-fallback.conf"
    fi

    if [ "$VIRTUALBOX" == "true" ]; then
        echo -n "\EFI\systemd\systemd-bootx64.efi" > "${ALISMNT}$ESP_DIRECTORY/startup.nsh"
    fi
}
...
function custom_shell_user() {
    ...
    if [ "$SYSTEMD_HOMED" == "true" -a "$USER" != "root" ]; then
        ...
        arch-chroot ${ALISMNT} chsh -s $CUSTOM_SHELL_PATH $USER
    fi
}
...
function desktop_environment() {
    ...
    arch-chroot ${ALISMNT} systemctl set-default graphical.target
}
...
function display_manager_gdm() {
    pacman_install "gdm"
    arch-chroot ${ALISMNT} systemctl enable gdm.service
}

function display_manager_sddm() {
    pacman_install "sddm"
    arch-chroot ${ALISMNT} systemctl enable sddm.service
}

function display_manager_lightdm() {
    pacman_install "lightdm lightdm-gtk-greeter"
    arch-chroot ${ALISMNT} systemctl enable lightdm.service
    user_add_groups_lightdm

    if [ "$DESKTOP_ENVIRONMENT" == "deepin" ]; then
        arch-chroot ${ALISMNT} sed -i 's/^#greeter-session=.*/greeter-session=lightdm-deepin-greeter/' /etc/lightdm/lightdm.conf
        arch-chroot ${ALISMNT} systemctl enable lightdm.service
    fi
}
...
function display_manager_lxdm() {
    pacman_install "lxdm"
    arch-chroot ${ALISMNT} systemctl enable lxdm.service
}
...
function provision() {
    ...
    (cd "$PROVISION_DIRECTORY" && cp -vr --parents . ${ALISMNT})
}

function vagrant() {
    pacman_install "openssh"
    create_user "vagrant" "vagrant"
    arch-chroot ${ALISMNT} systemctl enable sshd.service
    arch-chroot ${ALISMNT} ssh-keygen -A
    arch-chroot ${ALISMNT} sshd -t
}
...
function copy_logs() {
    ...
    if [ -f "$ALIS_CONF_FILE" ]; then
        ...
        local FILE="${ALISMNT}/var/log/alis/$ALIS_CONF_FILE"
    ...
    if [ -f "$ALIS_LOG_FILE" ]; then
        ...
        local FILE="${ALISMNT}/var/log/alis/$ALIS_LOG_FILE"
    ...
    if [ -f "$ALIS_ASCIINEMA_FILE" ]; then
        ...
        local FILE="${ALISMNT}/var/log/alis/$ALIS_ASCIINEMA_FILE"
